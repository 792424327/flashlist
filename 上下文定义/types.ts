...
export interface ListState {
  items: FlashListItem[];  // 待办事项列表
}

/**
 * TodoItem组件Props（前端使用）
 */
export interface TodoItemProps {
  item: FlashListItem;                               // 待办事项数据
  onUpdate?: (updates: UpdateItemRequest) => void;   // 更新回调
  onEnter?: (type: ItemType) => void;                // Enter键回调
  onDelete?: () => void;                             // 删除回调
  onIndent?: (direction: IndentDirection) => void;   // 缩进调整回调
  onToggleComplete?: () => void;                     // 完成状态切换回调
  isFocused?: boolean;                               // 是否聚焦
  onFocus?: () => void;                              // 聚焦回调
  isOverlay?: boolean;                               // 是否为拖拽预览
}
...
export const API_ENDPOINTS = {
  ...
  UPDATE_ITEM: (id: string) => `${API_BASE_URL}/items/${id}`,
  DELETE_ITEM: (id: string) => `${API_BASE_URL}/items/${id}`,
  REORDER_ITEMS: `${API_BASE_URL}/items/reorder`,
 ...
 * 创建待办事项时的必填字段
 */
export type FlashListItemCreate = Required<Pick<FlashListItem, 'text' | 'level' | 'type'>>;
 ...
 * 创建默认的待办事项
 */
export function createDefaultItem(overrides?: Partial<FlashListItem>): FlashListItem {
  return {
    id: '',  // 应由UUID生成器填充
    text: '',
    completed: false,
